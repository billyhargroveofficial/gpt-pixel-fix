...
Финальный контроль подтвердил это уже на физическом Pixel, без патча APK.
После явно разрешённого полного factory downgrade Pixel 9 Pro XL до
`BP2A.250605.031.A2` (Android 16, patch level `2025-06-05`) официальный
ChatGPT `1.2026.202` работал с Google WebView/Chrome `133.0.6943.137`.
В том же тяжёлом математическом чате прогон из 24 свайпов дал 655 кадров,
6 deadline-janky frames (`0.92%`), p50 `6 ms` и p99 `21 ms`. Ранее
официальный клиент в реальном тяжёлом чате на современном provider показывал
`87.06%` jank и p50 `150 ms`.
...
Точная физическая среда, метрики и фактически применённая блокировка
автообновлений описаны в
[`docs/physical-pixel-android16-webview133.md`](docs/physical-pixel-android16-webview133.md).
