...
Исходный ROM-контроль выполнялся без смены прошивки физического Pixel. Позже,
с отдельного явного разрешения владельца и после резервного копирования,
Pixel был полностью прошит официальным factory image Google
`BP2A.250605.031.A2` (Android 16, WebView/Chrome 133). Это не GrapheneOS:
физический тест GrapheneOS по-прежнему не выполнялся.

На stock Android 16 официальный, неизменённый ChatGPT `1.2026.202` после
свежего входа дал в реальном тяжёлом чате `0.92%` deadline jank, p50 `6 мс`
и p99 `21 мс` за 655 кадров/24 свайпа. Тем самым результат matched emulator
control был подтверждён на самом Pixel. Цена workaround — заморозка OS и
security-critical WebView на patch level `2025-06-05`.
